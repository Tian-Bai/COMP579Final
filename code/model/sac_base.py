...
parser.add_argument('--name', dest='envname', action='store', default='None')
parser.add_argument('--num', dest='envnum', action='store', type=int, default=-1)
...
class PolicyNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, action_range, hidden_size, activation=F.relu, init_w=3e-3, log_std_min=-20, log_std_max=2):
...
DETERMINISTIC=False
ENVLIST = ['Pendulum-v1', 'Humanoid-v4', 'HalfCheetah-v4', 'Hopper-v4', 'Walker2d-v4', 'Ant-v4']
ACTIONRANGELIST = [2.0, 0.4, 1.0, 1.0, 1.0, 1.0]

# choose env, and action range
if args.envnum != -1:
    ENV = ENVLIST[args.envnum]
    action_range = ACTIONRANGELIST[args.envnum]
elif args.envname != 'None':
    if args.envname not in ENVLIST:
        raise ValueError("incorrect environment name.")
    ENV = args.envname
    for i in range(len(ACTIONRANGELIST)):
        if ENVLIST[i] == args.envname:
            action_range = ACTIONRANGELIST[i]

env = NormalizedActions(gym.make(ENV, render_mode='rgb_array'))
...
policy_net = PolicyNetwork(state_dim, action_dim, action_range, hidden_dim, activation=F.relu).to(device)
