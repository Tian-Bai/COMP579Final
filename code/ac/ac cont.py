...
# if is continuous, get the dimension of action
if not CONTINUOUS_ACTION:
    action_dim = sample_env.action_space.n
else:
    action_dim = sample_env.action_space.shape[0]
    HIGH = torch.tensor(sample_env.action_space.high)
    LOW = torch.tensor(sample_env.action_space.low)
...
if not CONTINUOUS_ACTION:
    ...
    class Actor(nn.Module):
        def __init__(self, hidden_dim=16):
            ...
            self.mu_hidden = nn.Linear(state_dim, hidden_dim)
            self.mu_output = nn.Linear(hidden_dim, action_dim)
            self.sigma_hidden = nn.Linear(state_dim, hidden_dim)
            self.sigma_output = nn.Linear(hidden_dim, action_dim)

        def forward(self, s):
            mu = self.mu_hidden(s)
            mu = self.mu_output(F.tanh(mu))
            sigma = self.sigma_hidden(s)
            sigma = self.sigma_output(F.tanh(sigma)) 
...
class Agent():
    ...
    if not CONTINUOUS_ACTION:
        ...
        def select_action(self, state):
            state = torch.from_numpy(state).float()
            [mu, sigma], val = self.actor(state), self.value(state)
            ...
            if sigma == 0:
                ...
                action = torch.clip(action, LOW, HIGH)
            self.steps.append(SavedAction(log_prob, val))
