...
actor_past_grad = [] # previous gradients of actor model
...
def finish_episode():
    ...
    actor_past_grad.append(actor_grad)
...
def finish_step(update_time, lr=1e-4):
    ...
    '''
    global value_past_grad, actor_past_grad, steps, rewards
    ...
    for i_update in range(update_time):
        ...
        with torch.no_grad():
            ...
            for i, p in enumerate(actor.parameters()):
                new_p = p - lr * (actor_mu[i] - actor_past_grad[t][i] + p.grad)
                p.copy_(new_p)
